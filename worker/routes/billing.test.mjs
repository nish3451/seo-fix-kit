...
test("signed refund.succeeded closes a paid Fix Pack exactly once and keeps refund provenance", async () => {
  ...
  // keeps the original refunded_at (COALESCE arm) and never un-refunds.
...
function first(sql, values, env) {
  ...
  if (sql.includes("FROM offer_entitlements") && sql.includes("owner_email = ?")) {
    ...
    // Mirror the SQL text: only queries that say `revoked_at IS NULL` (the
    // active-entitlement lookup) may ignore revoked rows. A mutation dropping
    // that predicate then changes mock behavior verbatim instead of being
    // masked by a hardcoded filter.
...
function run(sql, values, env) {
  ...
  // Payment-failure UPDATE arm. The mock mirrors the CASE/COALESCE expressions
  // from the SQL text so mutations to billing.js change behavior verbatim.
  if (sql.includes("UPDATE fix_requests") && sql.includes("'payment_failed'")) {
    const row = env.fixRequests.find((item) => item.id === values[3]);
    if (row) {
      row.status = sql.includes("WHEN paid_at IS NOT NULL THEN status")
        ? (row.paid_at ? row.status : "payment_failed")
        : "payment_failed";
  ...
  // Refund-failure UPDATE arm.
  if (sql.includes("UPDATE fix_requests") && sql.includes("'refund_failed'")) {
    const row = env.fixRequests.find((item) => item.id === values[4]);
    if (row) {
      row.status = sql.includes("WHEN status = 'refunded' THEN status")
        ? (row.status === "refunded" ? row.status : "refund_failed")
        : "refund_failed";
  ...
  if (sql.includes("UPDATE fix_requests") && sql.includes("dispute_event = ?")) {
    const row = env.fixRequests.find((item) => item.id === values[3]);
    if (row) {
      row.status = sql.includes("WHEN status = 'delivered' THEN status")
        ? (row.status === "delivered" ? row.status : "disputed")
        : "disputed";
