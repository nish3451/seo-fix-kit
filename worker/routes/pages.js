    ...
    <main>
      ...
      <section class="grid" aria-label="Sample audit outcome">
        ...
        <article class="panel">
          ...
          <p>No thin-content fix is queued: the engine guards the warning with rendered-word evidence. Issues that are real still surface — noindex, canonical conflict, missing share image, missing schema — each with an exact snippet.</p>
        </article>
      </section>
      <section class="panel proof">
        ...
        <p><strong>Guarded finding (${escapeHtml(DEMO_PROOF.guard.title)}):</strong></p>
        <code>- Evidence: ${escapeHtml(DEMO_PROOF.guard.evidence)}
- Why: ${escapeHtml(DEMO_PROOF.guard.why)}
- Fix: ${escapeHtml(DEMO_PROOF.guard.fix)}</code>
...
${DEMO_PROOF.repairPlan.map((item) => `          <li><strong>${escapeHtml(item.severity)}</strong> ${escapeHtml(item.title)} — ${escapeHtml(item.fix)}${item.snippet ? `\n            <code>${escapeHtml(demoProofSnippet(item.snippet, origin))}</code>` : ""}</li>`).join("\n")}
      ...
      <section>
        ...
        <p>On this test page the static-looking warning was thin content: ${DEMO_PROOF.measured.staticWordCount} visible words in the raw response vs ${DEMO_PROOF.measured.renderedWordCount} rendered words. The same raw-vs-rendered comparison guards missing-H1 and missing-link warnings on sites that paint content with JavaScript.</p>
      ...
      <section>
        ...
        <p>This is a public proof page, not a public anonymous audit. Full reports run inside the private beta after secure email access and, for deeper crawls, site verification. The proof excerpt above is the engine's real output for the test page, not a preview of your site's results. The demo does not promise rankings, traffic, indexing, revenue, AI citations, or live answer-engine visibility. It shows the product standard: prove the issue, avoid false positives, ask for approval, then rerun the same measurement after the fix.</p>
      </section>
