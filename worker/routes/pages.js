...
function methodologyHtml(origin) {
  return publicProductPageHtml({
    ...
    body: `
      ...
      <section class="band">
        <h2>Why not just use Zaatar to ship fixes as pull requests?</h2>
        <p>Zaatar (<a href="https://zaatar.ai/" rel="nofollow noopener" target="_blank">zaatar.ai</a>) is the closest competitor on delivery: launched in July 2026 as "AI that opens PRs on your repo to fix your SEO," it runs a full audit every week, turns what is broken into a prioritized queue, and opens technical fixes — page speed, indexing, schema, and meta tags — as pull requests in your repository for you to approve. Plans run $59–$419 per month with no free tier, and nothing ships without you. If what you want is the fix landed as a PR today, that is a real product doing real delivery, and this page will not pretend otherwise.</p>
        <p>The honest gap on our side: SEO Fix Kit generates fix snippets when the engine can build one, turns approved actions into implementation packs, then stops at the handoff. It does not open pull requests against your repository — GitHub PR delivery is planned, not live, so no SEO Fix Kit surface will claim your fixes ship themselves.</p>
        <p>The slower loop is the safer one for sites that do not want automated repo access. Before any fix is proposed, the rendered-vs-static check proves the fault exists on the browser-visible page, so neither a human nor an agent "fixes" something the browser already renders correctly. Every queue item carries source proof, a suggested action, an approval gate, and an acceptance check; and a rerun after your fix ships issues a receipt that says fixed, still-open, new, or regressed instead of stopping at a re-crawl confirming the page parsed. Because no agent holds write access to your repo, there is also no automated path that can push code while you are not looking.</p>
        <p>Honest differences in the other direction: Zaatar has no free tier by design while the anonymous one-page check here needs no account, and Zaatar's homepage promises rankings usually move two to four weeks after Google recrawls — a timing promise this product does not make. No live AI-engine sampling, no AI citation monitoring, and no ranking guarantees — those limits apply to this comparison too.</p>
      </section>
      <section class="band">
