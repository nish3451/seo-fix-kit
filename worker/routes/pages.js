...
function methodologyHtml(origin) {
  return publicProductPageHtml({
    ...
    body: `
      ...
      <section class="band">
        <h2>Why not just use GEO Wiki's free AI Crawler Access Checker?</h2>
        <p>GEO Wiki, an open encyclopedia for generative engine optimization, publishes a <a href="https://geo.wiki/tools/ai-crawler-access" rel="nofollow noopener" target="_blank">free AI Crawler Access Checker</a> that needs no account: enter a domain and it evaluates 26 AI crawler tokens against robots.txt parsed per RFC 9309, quoting the exact rule line behind every verdict, then probes each probeable bot user agent (24 of 26 — Google-Extended and Applebot-Extended are robots.txt-only tokens) against the same page and compares responses with a browser baseline to surface UA-keyed edge rules. The tool page states "Free, no account needed — 5 checks a day" (15 with sign-in) and was last updated 18 August 2026. Sibling free tools cover citability, schema markup, and llms.txt generation. For a fast map of how AI crawlers see a domain, that is a fair choice.</p>
        <p>The difference starts where the map ends. A crawler-access checker tells you what robots.txt declares and what the edge serves each bot UA; SEO Fix Kit turns each proven page-level finding into a persistent repair-queue item with severity, source proof, a suggested action, approval state, and an acceptance check, then re-measures after your fix ships so the queue says fixed, still-open, new, or regressed. For sites that need fixes rather than another access map, that repair queue plus rerun proof is the product.</p>
        <p>One honest note in both directions: as of August 2026, GEO Wiki's <a href="https://geo.wiki/products" rel="nofollow noopener" target="_blank">GEO product directory</a> listed four products — Profound, Peec AI, LLMrefs, and Otterly.AI — and SEO Fix Kit is not among them yet; a correction-path email to support@geo.wiki is prepared and awaiting manual submission by the account owner. The boundary stays the same as every comparison above: No live AI-engine sampling, no AI citation monitoring, and no ranking guarantees.</p>
      </section>
      <section class="band">
