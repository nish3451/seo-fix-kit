...
- AI Answer Readiness checks derived from rendered content depth, helpful schema, canonical/internal-link clarity, question-led structure, sitemap context, and optional `/llms.txt` reachability; no live AI-engine sampling or citation monitoring.
...
- Self-serve Developer API keys, `/v1/audits` JSON endpoints, project-style verified sites, safe `repair_queue` issue status, and audit/repair-action lifecycle webhooks.
...
- `/api/reports/:id/repair-queue`, `/api/reports/:id/repair-actions`, and `/api/reports/:id/repair-actions/:actionId` power private proof-backed queue state and approval-safe action records
