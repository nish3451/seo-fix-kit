...
- `/api/funnel-event` records privacy-safe first-party activation funnel events (page view, access form shown, access-request success/failure, and public CTA clicks) with only an allow-listed event name, a page path, and a timestamp — no email, company, IP, or query strings; `/admin/funnel-summary` is the read-only, admin-token-protected event-count summary (90-day retention)
...
The public private-beta funnel records first-party activation events only: `page_view`, `access_form_shown`, `access_request_success`, `access_request_failure`, and `cta_activation`. Events carry only an allow-listed event name, a page path, and a timestamp — never email, company, IP address, user agent, referrer, or URL query strings (query strings are rejected server-side and stripped client-side). Access-request success/failure is recorded server-side by `/api/access/request`; page view, form-shown, and CTA events are emitted by the first-party beacon on `/` and the server-rendered `/demo`, `/methodology`, `/packages`, and `/check` pages. Rows are deleted after 90 days by the scheduled cleanup, and the `/privacy` page documents these retention and consent boundaries. The read-only summary lives at `/admin/funnel-summary` behind the admin token.
