...
test("README local SEO claim matches NAP, LocalBusiness schema, and citation checks", () => {
  ...
  assert.match(liveSection, /Google Business Profile URL, local keywords, and citation URLs/i);
...
test("README platform audit claim matches schema, faceted, archive, and plugin checks", () => {
  ...
  assert.match(liveSection, /for detected stores\/CMS pages/i);
...
// Lane-1 promise audit, current pass. A fresh audit of every 'What is live in
// this repo' bullet plus a live spot-check of /demo, /methodology, and
// /packages confirmed the claims below are backed by code today, but the
// sub-claims had no regression pin of their own. Each pin ties the README
// wording to the exact code that must keep backing it.
...
test("README anonymous one-page check claim keeps browser proof, guards, rate windows, nothing stored, and handoff", () => {
  ...
  assert.match(publicCheckSource, /nothing about your check is stored/, "the page states nothing is stored");
